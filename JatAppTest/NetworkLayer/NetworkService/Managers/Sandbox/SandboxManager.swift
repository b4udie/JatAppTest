...
protocol SandboxManagerProtocol {
    ...
    func getPerson(completion: @escaping (Result<Person, ErrorType>) -> Void)
}
...
final class SandboxManager: CommonNetworkManager, SandboxManagerProtocol {
    ...
    func getText(completion: @escaping (Result<String, ErrorType>) -> Void) {
        ...
        session.request(request).validate().responseDecodable(of: TextModel.self) { response in
    ...
    func getPerson(completion: @escaping (Result<Person, ErrorType>) -> Void) {
        let request = SandboxRouter.getPerson.asURL()
        
        session.request(request).validate().responseDecodable(of: PersonModel.self) { response in
